class CaixaDaLanchonete {
    ...
    checkExtra(itens, formaDePagamento) {
        ...
        Object.keys(this.menu).map((name, index) => {
            ...
            new_menu[name].principal = this.menu[name].descricao.includes(
                "extra"
            )
                ? false
                : true;
            new_menu[name].requisito = this.menu[name].descricao.includes(
                "extra"
            )
                ? lower(this.menu[name].descricao.match(/extra do (.*?)\)/)[1])
                : "";
    ...
    calcularValorDaCompra(formaDePagamento, itens) {
        ...
        if (sum == 0 && formaDePagamento == "dinheiro")
